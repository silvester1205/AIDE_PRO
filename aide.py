...
class ExportTab(QWidget):
    ...
    def _export_wide(self):
        ...
        # Row 0 = header (field names), rows 1+ = data
        if len(df) < 2:
        ...
        data = df.iloc[1:].copy()
        data.columns = header
        # Group by first column (typically author+year), collapse arms into pivot
        first_col = header[0]
        # Use first column as study identifier; group rows and collect as list
        # For simplicity: group by all study-level columns that have identical values per study
        # Actually, just group by first column and collapse value combinations
        try:
            # Find which columns are likely study-level (same value for a study)
            ...
            for study_name, group in grouped:
                ...
                # Fill remaining arm columns if some studies have more arms
                row['_n_arms'] = arm_num
            ...
            # Drop the _n_arms helper
            out = out.drop(columns=['_n_arms'])
        except Exception as e:
            # Fallback: just export as-is
